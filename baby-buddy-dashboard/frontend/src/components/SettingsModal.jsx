...
export default function SettingsModal({ onClose, onChanged }) {
  const [section, setSection] = useState("babies");
  ...
  const beginEdit = (child) => {
    setResetChild(null); setResetResult(null); setEditingId(child.id);
    setProfile({
  ...
  const beginReset = (child) => {
    setEditingId(null); setResetChild(child); setConfirmation(""); setIncludeAudit(true); setResetResult(null); setError(""); setMessage("");
  };

  const runReset = async () => {
    if (!resetChild) return;
    setBusyId(resetChild.id); setError(""); setMessage("");
    try {
  ...
  const saveTimerReminders = async () => {
    setSavingTimerReminders(true); setError(""); setMessage("");
    try {
    ...
    finally { setSavingTimerReminders(false); }
  };

  const saveMedicationAlerts = async () => {
    setSavingAlerts(true); setError(""); setMessage("");
    try {
  ...
  return (
    <Modal title="Ajustes" onClose={onClose} maxWidth={700}>
      <div className="settings-tabs" role="tablist" aria-label="Secciones de ajustes">
        <button type="button" className={section === "babies" ? "active" : ""} onClick={() => switchSection("babies")}><Icons.Baby /><span>Bebés</span></button>
        <button type="button" className={section === "timers" ? "active" : ""} onClick={() => switchSection("timers")}><Icons.Timer /><span>Recordatorios</span></button>
        <button type="button" className={section === "medication" ? "active" : ""} onClick={() => switchSection("medication")}><Icons.Pill /><span>Medicación</span></button>
      </div>
      ...
      {section === "timers" && (
        <div className="settings-tab-panel">
          <section className="settings-child-card is-enabled">
            <div className="settings-child-head">
              <div className="settings-child-avatar"><Icons.Timer /></div>
              <div className="settings-child-title"><strong>¿Te has olvidado de parar?</strong><span>Avisa si una toma o el tiempo boca abajo parecen haberse quedado activos.</span></div>
              <label className="settings-switch">
                ...
                <span className="settings-switch-track"><span /></span><em>{timerReminders.enabled ? "Activo" : "Desactivado"}</em>
              </label>
            </div>
        ...
        <div className="settings-tab-panel">
          <section className="settings-child-card is-enabled">
            <div className="settings-child-head">
              ...
              <div className="settings-child-title"><strong>Avisos de medicación</strong><span>Configura cuándo y por dónde avisar de una pauta activa.</span></div>
              <label className="settings-switch">
                ...
                <span className="settings-switch-track"><span /></span><em>{medicationAlerts.enabled ? "Activos" : "Desactivados"}</em>
              </label>
            </div>
            <div className="settings-editor">
              <div className="settings-fields-grid">
                <label><span>Avisar antes de la dosis</span><FormInput type="number" min="0" max="120" value={medicationAlerts.minutes_before ?? 20} onChange={(e) => setMedicationAlerts({ ...medicationAlerts, minutes_before: Number(e.target.value || 0) })} /><small>Minutos. Recomendado: 20.</small></label>
              </div>
      ...
      {section === "babies" && (
        <div className="settings-tab-panel">
          <div className="settings-intro">
            <div className="settings-intro-icon"><Icons.Baby /></div>
            <div><strong>Bebés de Baby Buddy</strong><p>La lista se obtiene automáticamente del servidor. Ocultar un bebé no borra ningún dato.</p></div>
          </div>
                ...
                return (
                  <section key={child.id} className={`settings-child-card${child.enabled ? " is-enabled" : " is-disabled"}`}>
                    <div className="settings-child-head">
                      ...
                      <div className="settings-child-title"><strong>{displayName(child)}</strong><span>{child.birth_date ? `Nacimiento: ${child.birth_date}` : "Fecha de nacimiento sin definir"}</span></div>
                      <label className="settings-switch" title={child.enabled ? "Visible y monitorizado" : "Oculto y sin avisos"}>
                        <input type="checkbox" checked={Boolean(child.enabled)} disabled={isBusy || (child.enabled && enabledCount <= 1)} onChange={() => toggleChild(child)} />
                        <span className="settings-switch-track"><span /></span><em>{child.enabled ? "Activo" : "Oculto"}</em>
                      </label>
                    </div>

                    <div className="settings-child-actions">
                      <button onClick={() => isEditing ? setEditingId(null) : beginEdit(child)} disabled={isBusy}><Icons.Pencil /> {isEditing ? "Cerrar edición" : "Editar datos"}</button>
                      <button className="settings-danger-link" onClick={() => beginReset(child)} disabled={isBusy}><Icons.Trash /> Borrar historial de pruebas</button>
                    </div>

                    {isEditing && (
                      <div className="settings-editor">
                        <div className="settings-fields-grid">
                          <label><span>Nombre</span><FormInput value={profile.first_name} onChange={(e) => setProfile({ ...profile, first_name: e.target.value })} /></label>
                          <label><span>Apellidos</span><FormInput value={profile.last_name} onChange={(e) => setProfile({ ...profile, last_name: e.target.value })} /></label>
                          <label><span>Fecha de nacimiento</span><FormInput type="date" value={profile.birth_date || ""} onChange={(e) => setProfile({ ...profile, birth_date: e.target.value })} /></label>
                          <label><span>Hora de nacimiento</span><FormInput type="time" value={(profile.birth_time || "").slice(0, 5)} onChange={(e) => setProfile({ ...profile, birth_time: e.target.value })} /></label>
                        </div>
                        <p className="settings-help">Esto modifica el perfil real de Baby Buddy. El ID interno del bebé no cambia.</p>
                        <div className="settings-editor-actions"><button onClick={() => setEditingId(null)}>Cancelar</button><button className="settings-primary" onClick={() => saveProfile(child)} disabled={isBusy || !profile.first_name.trim() || !profile.birth_date}>{isBusy ? "Guardando…" : "Guardar datos"}</button></div>
                      </div>
                    )}
                    ...
                    {isResetting && (
                      <div className="settings-reset-panel">
                        ...
                        <p>Se eliminarán las entradas de <strong>{displayName(child)}</strong>: tomas, sueño, pañales, medicamentos, temperaturas, peso, altura, notas, bombeos, tiempo boca abajo y temporizadores. <strong>El perfil del bebé no se borra.</strong></p>
                        <p>Las existencias de Grocy no se modifican durante esta limpieza.</p>
                        <label className="settings-check-row"><input type="checkbox" checked={includeAudit} onChange={(e) => setIncludeAudit(e.target.checked)} /><span>Borrar también el historial de auditoría de este bebé</span></label>
                        <label className="settings-confirm-label">Para confirmar, escribe <code>BORRAR {child.first_name || "Bebé"}</code><FormInput value={confirmation} autoComplete="off" onChange={(e) => setConfirmation(e.target.value)} placeholder={`BORRAR ${child.first_name || "Bebé"}`} /></label>
                        <div className="settings-editor-actions"><button onClick={() => { setResetChild(null); setResetResult(null); }}>Cancelar</button><button className="settings-danger-button" onClick={runReset} disabled={isBusy || confirmation !== `BORRAR ${child.first_name || "Bebé"}`}>{isBusy ? "Borrando…" : "Borrar historial"}</button></div>
                        {resetResult && <div className="settings-reset-result"><strong>{resetResult.total_deleted || 0} entradas eliminadas</strong>{resetResult.audit_deleted > 0 && <span>{resetResult.audit_deleted} registros de auditoría eliminados</span>}{(resetResult.errors || []).length > 0 && <span className="settings-reset-warning">Algunas categorías dieron error: {resetResult.errors.join(" · ")}</span>}</div>}
                      </div>
                    )}
                  </section>
                );
          ...
          <div className="settings-footer-note">Los bebés nuevos que aparezcan en Baby Buddy se detectarán automáticamente. Debe quedar al menos uno activo en la app.</div>
        </div>
      )}
    </Modal>
  );
